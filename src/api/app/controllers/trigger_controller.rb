class TriggerController < ApplicationController
  ...
  def release
    ...
    manual_release_targets.each do |release_target|
      raise NoPermissionForProject.setup('not_permission', 403, "#{release_target.target_repository.project.name} project is not writable for user") unless policy(release_target.target_repository.project).update?
  ...
  def require_valid_token
    ...
    if @token.package
      ...
      opts = if @token.class == Token::Rebuild
             ...
             elsif @token.class == Token::Release
               { use_source: true,
